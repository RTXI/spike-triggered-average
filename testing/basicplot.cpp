...
BasicPlot::BasicPlot(QWidget *parent) : QwtPlot(parent) {
  setFrameStyle(QFrame::NoFrame);
  setLineWidth(0);
//  setCanvasLineWidth(2);
  ((QwtPlotCanvas *)canvas())->setLineWidth(2);
  ...
  // enable zooming
  Zoomer *zoomer = new Zoomer(((QwtPlotCanvas *)canvas()));
...
void BasicPlot::setAxes(double xmin, double xmax, double ymin, double ymax) {
...
QSize BasicPlot::sizeHint() const {
